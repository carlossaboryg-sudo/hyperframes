...
function getDocumentStyleSignature(doc: Document): string {
  const styleText = Array.from(doc.querySelectorAll("style"))
    .filter((style) => style.id !== "hyper-shader-loading-styles")
...
function ensureSnapshotLoadingStyles(doc: Document): void {
  if (doc.getElementById("hyper-shader-loading-styles")) return;
  const style = doc.createElement("style");
  style.id = "hyper-shader-loading-styles";
  style.textContent = [
    "@keyframes hyperShaderLoaderBreathe {",
    "0%,100% { transform: scale(1); filter: drop-shadow(0 0 6px rgba(6,227,250,0)); }",
    "50% { transform: scale(1.04); filter: drop-shadow(0 0 14px rgba(79,219,94,0.35)); }",
    "}",
    "@keyframes hyperShaderLoaderLeft {",
    "0%,100% { opacity: 1; transform: translateY(0); }",
    "50% { opacity: .45; transform: translateY(.5px); }",
    "}",
    "@keyframes hyperShaderLoaderRight {",
    "0%,100% { opacity: .45; transform: translateY(.5px); }",
    "50% { opacity: 1; transform: translateY(0); }",
    "}",
    "@media (prefers-reduced-motion: reduce) {",
    "[data-hyper-shader-loading] svg,",
    "[data-hyper-shader-loading] [data-hf-loader-left],",
    "[data-hyper-shader-loading] [data-hf-loader-right] {",
...
function createSnapshotLoadingOverlay(
  ...
  ensureSnapshotLoadingStyles(doc);
  ...
  panel.style.cssText = [
    "display:grid",
    "grid-template-rows:156px 72px 44px 44px 54px",
  ...
  mark.setAttribute("width", "156");
  mark.setAttribute("height", "156");
  ...
  mark.style.cssText = [
    "display:block",
    "animation:hyperShaderLoaderBreathe 2.4s ease-in-out infinite",
    "will-change:transform,filter",
    "user-select:none",
    "-webkit-user-select:none",
  ].join(";");
  mark.innerHTML = [
    '<path data-hf-loader-left d="M10.1851 57.8021L33.1145 73.8313C36.2202 75.9978 41.5173 73.5433 42.4816 69.4984L51.7611 30.4271C52.7253 26.3822 48.5802 23.9277 44.4602 26.0942L13.917 42.1235C6.96677 45.7676 4.97564 54.1579 10.1851 57.8021Z" fill="url(#hyper-shader-loader-grad-left)" style="transform-origin:30% 50%;animation:hyperShaderLoaderLeft 2.4s ease-in-out infinite;will-change:opacity,transform"/>',
    '<path data-hf-loader-right d="M87.5129 57.5141L56.9696 73.5433C52.8371 75.7098 48.7046 73.2553 49.6688 69.2104L58.9483 30.1391C59.9125 26.0942 65.2097 23.6397 68.3154 25.8062L91.2447 41.8354C96.4668 45.4796 94.4631 53.8699 87.5129 57.5141Z" fill="url(#hyper-shader-loader-grad-right)" style="transform-origin:70% 50%;animation:hyperShaderLoaderRight 2.4s ease-in-out infinite;will-change:opacity,transform"/>',
    "<defs>",
...
export function init(config: HyperShaderConfig): GsapTimeline {
  ...
  const paintSettledSceneState = (currentTime: number): void => {
    ...
    scenes.forEach((sceneId, index) => {
      const scene = document.getElementById(sceneId);
      if (!scene) return;
      scene.style.opacity = index === visibleIndex ? "1" : "0";
  ...
  const applyFallbackTransition = (cache: CachedTransition, progress: number): void => {
    ...
    fromScene.style.visibility = "visible";
    toScene.style.visibility = "visible";
    fromScene.style.opacity = String(1 - eased);
    toScene.style.opacity = String(eased);
  };
